...
.env-symfony:
	...
	then\
		...
		echo "\033[107;34m What is you project's name? (no spaces) : \033[0m"; \
		read -p "" PROJECT_NAME \
		touch .env.local;\
		echo "### Only used for docker containers ###" > .env.local;\
		echo "PROJECT_NAME=$${PROJECT_NAME}" >> .env.local;\
		echo "### ------------------------------- ###" >> .env.local;\
		echo "" >> .env.local;\
		echo "DATABASE_URL=mysql://root:@db_docker_$${PROJECT_NAME}:3306/db_$${PROJECT_NAME}?serverVersion=mariadb-10.5.8" >> .env.local;\
...
mercure-server: ## Starts Mercure server
mercure-server:
	SERVER_NAME=:3000 MERCURE_PUBLISHER_JWT_KEY='!ChangeMe!' MERCURE_SUBSCRIBER_JWT_KEY='!ChangeMe!' CORS_ALLOWED_ORIGINS=* ./mercure run -config Caddyfile.dev